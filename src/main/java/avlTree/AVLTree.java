...
public class AVLTree
{
	...
	public AVLNode search(AVLNode sNode, int key)
	{
		if (sNode == null)
		{
			System.out.println("未找到结点Node[" + key + "]");
			return null;
		}
		if (key == sNode.Data)
			System.out.println("已找到结点Node[" + sNode.Data + "] Height="
					+ sNode.Height);
	...
	public AVLNode insert(AVLNode sNode, int key)
	{
		if (sNode == null)
		{
			...
			System.out.println("结点Node[" + key + "]插入成功");
		}
		else if (key == sNode.Data)
			System.out.println("结点Node[" + sNode.Data + "]插入失败：已存在，不允许重复插入");
		else if (key < sNode.Data)
		{
	...
	public AVLNode delete(AVLNode sNode, int key)
	{
		if (sNode == null)
		{
			System.out.println("删除失败：未找到结点Node[" + key + "]");
			return null;
		}
		if (key == sNode.Data)
		{
	...
	public void presuccessor(AVLNode sNode, int key)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			{
				if (i == 0)
					System.out.println("结点Node[" + key + "]无前驱结点");
				else
					System.out.println("结点Node[" + key + "]的前驱结点为Node["
							+ list.get(i - 1).Data + "]");
				break;
			}
		}
	...
	public void successor(AVLNode sNode, int key)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			{
				if (i == list.size() - 1)
					System.out.println("结点Node[" + key + "]无后继结点");
				else
					System.out.println("结点Node[" + key + "]的后继结点为Node["
							+ list.get(i + 1).Data + "]");
				break;
			}
		}
